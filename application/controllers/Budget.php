...
class Budget extends BaseController
{
	...
	public function budgetList()
    {
		...
		$data['budgets']	= $this->budget_model->getListBudget();
			
		
		$this->loadViews("budget/list_budget", $this->global, $data);
		
    }
	
	public function setup_budget()
    {
        $this->global['pageTitle'] = 'Setup Master Budget';
		...
		if(true)
        {
			...
			$this->loadViews("budget/setup", $this->global, $data);
	...
	public function detail_budget($name)
    {
        $this->global['pageTitle'] = 'Setup Master Budget';
		...
		if(true)
        {
			...
			$data['budget']	= $this->budget_model->getDetailBudget($name);
			
			
			$this->load->View("budget/budget_detail", $data);
	...
	public function set_detail_budget($name)
    {
        $this->global['pageTitle'] = 'Setup Master Budget';
		...
		$this->load->View("budget/set_budget_detail", $data);
	
	
	
    }
	
	public function usage_detail_budget($oppID, $bName)
    {
	...
	public function spending_form($oppID, $bID)
    {
		...
		$data['budgets']	= $this->budget_model->getBudgetInfo($oppID,$bID);
		
		$data['oppID'] 	= $oppID;
		$data['bID']	= $bID;
		$this->load->View("budget/spending_form", $data);
	
	
    }
	
	public function doSetupBudget()
    {
		...
		if(true)
        {
			...
			$p_order = $this->input->post('order');
			$p_name  = $this->input->post('type');
			$c_order = $this->input->post('comp_order');
			$c_name	 = $this->input->post('comp');
			$set_name	 = $this->input->post('set_name');
			
			
			$result = $this->budget_model->setupBudget($p_order, $p_name, $c_order, $c_name, $set_name);
	...
	public function doAddSpending($source)
    {
		...
		{
			$config['upload_path']          = 'docs/budget';
			...
			$balance = $this->budget_model->checkBalance($cost, $oppbID);
			...
			$result = $this->budget_model->addNewSpending($spending, $file, $balance, $date);
