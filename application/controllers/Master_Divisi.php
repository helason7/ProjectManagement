 ...
 */
class Master_Divisi extends BaseController
{
    /**
	...
	public function delete_data($id){
	    ...
	    $id = array('ID' => $id);
	    $this->load->model('divisi_model');
	    $this->divisi_model->Delete('M_DIVISION', $id);
	    redirect('Master_Divisi/divisiList/');
	}
  ...
  // 		if($this->isAdmin() == true){
	 ...
	 //    $id = array('ID' => $id);
	 //    $this->load->model('divisi_model');
	 //    $div = $this->divisi_model->GetWhere('M_DIVISION', $id);
	 //    $data = array(
	 //    	'NAME'			=> $div[0]['NAME'],
	 //    	'SUPERIOR_DIV'	=> $div[0]['SUPERIOR_DIV'],
	 //    	);
		// $data['divs']	= $this->divisi_model->getListDivisi();

  // 		$this->loadViews("divisi/edit_form", $this->global, $data);
  // 		} else
  //         {
  //              redirect('/Main');
  //         }
  //     }
	...
	public function getDivById($id){
		 $this->global['pageTitle'] = 'Dashboard';
  		...
  		if($this->isAdmin() == true){
  			$data['divisis'] = $this->divisi_model->getListDivisiById($id);
  			$data['divs'] = $this->divisi_model->getListDivisi();
  		$this->loadViews("divisi/edit_form", $this->global, $data);
  		} else
          {
	...
	public function doEditDivisi($divId)
	{
		if($this->isAdmin() == TRUE)
        {
			$name = $this->input->post('name');
			$headdiv  = $this->input->post('headdiv');
			
			$array = array('NAME'=> $name, 'SUPERIOR_DIV'=> $headdiv);
			
			$result = $this->divisi_model->editDivisi($array, $divId);
			
			if($result > 0)
			{
				$this->session->set_flashdata('success', 'Divisi Set Submitted Successfully');
			}
			else
			{
				$this->session->set_flashdata('error', 'Update Divisi failed');
			}
	...
	public function divisiList()
    {
		...
		$data['divisis']	= $this->divisi_model->getListDivisi();
		
		$this->loadViews("divisi/list_divisi", $this->global, $data);
		
    }
	
	public function setup_divisi()
    {
        $this->global['pageTitle'] = 'Setup Master Divisi';
		...
		if(true)
        {
			...
			$this->loadViews("divisi/setup", $this->global, $data);
	...
	public function detail_divisi($name)
    {
        $this->global['pageTitle'] = 'Setup Master Divisi';
		...
		if(true)
        {
			...
			$data['divisi']	= $this->divisi_model->getDetailDivisi($name);
			
			
			$this->load->View("divisi/divisi_detail", $data);
	...
	public function set_detail_divisi($name)
    {
        $this->global['pageTitle'] = 'Setup Master Divisi';
		...
		$this->load->View("divisi/set_divisi_detail", $data);
	
	
	
    }
	
	public function usage_detail_divisi($oppID, $bName)
    {
		...
		$data['divisis']	= $this->divisi_model->getDivisiUsage($oppID,$bName);
		
		
		$this->load->View("divisi/usage_divisi_detail", $data);
	...
	public function spending_form($oppID, $bID)
    {
		...
		$data['divisis']	= $this->divisi_model->getDivisiInfo($oppID,$bID);
		
		$data['oppID'] 	= $oppID;
		$data['bID']	= $bID;
		$this->load->View("divisi/spending_form", $data);
	
	
    }
	
	public function doSetupDivisi()
    {
		...
		if(true)
        {
			...
			$result = $this->divisi_model->setupDivisi($array);
	...
	public function doAddSpending($source)
    {
		...
		{
			$config['upload_path']          = 'docs/divisi';
			...
			//check remaining divisi
			$balance = $this->divisi_model->checkBalance($cost, $oppbID);
			...
			$result = $this->divisi_model->addNewSpending($spending, $file, $balance, $date);
