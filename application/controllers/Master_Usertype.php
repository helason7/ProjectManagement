 ...
 */
class Master_Usertype extends BaseController
{
    /**
    ...
    public function index()
    {
		...
		if($this->isAdmin() == true)
        {
			...
			redirect('Usertype/usertypeList/');
	...
	public function usertypeList()
    {
		...
		$data['usertypes']	= $this->usertype_model->getListUsertype();
			
		
		$this->loadViews("usertype/list_usertype", $this->global, $data);
		
    }
	
	public function setup_usertype()
    {
        $this->global['pageTitle'] = 'Setup Master Usertype';
		...
		if(true)
        {
			...
			$data['users']	= $this->usertype_model->getListUsertype();
			$data['divs'] = $this->divisi_model->getListDivisi();
			$this->loadViews("usertype/setup", $this->global, $data);
	...
	public function delete_data($id){
	    ...
	    $id = array('ID' => $id);
	    $this->load->model('usertype_model');
	    $this->usertype_model->Delete('M_USER_TYPE', $id);
	    redirect('Master_Usertype/UsertypeList/');
	}
	...
	public function update_data($id){
		 $this->global['pageTitle'] = 'Dashboard';
  		...
  		if($this->isAdmin() == true){
			$data['users']	= $this->usertype_model->getListUsertypeById($id);
			$data['divs'] = $this->divisi_model->getListDivisi();
  		$this->loadViews("usertype/edit_form", $this->global, $data);
  		} else
          {
               redirect('/Main');
          }
	...
	public function detail_usertype($name)
    {
        $this->global['pageTitle'] = 'Setup Master Usertype';
		...
		if(true)
        {
			...
			$data['usertype']	= $this->usertype_model->getDetailUsertype($name);
			
			
			$this->load->View("usertype/usertype_detail", $data);
	...
	public function set_detail_usertype($name)
    {
        $this->global['pageTitle'] = 'Setup Master Usertype';
		...
		$this->load->View("usertype/set_usertype_detail", $data);
	
	
	
    }
	
	public function usage_detail_usertype($oppID, $bName)
    {
		...
		$data['usertypes']	= $this->usertype_model->getUsertypeUsage($oppID,$bName);
		
		
		$this->load->View("usertype/usage_usertype_detail", $data);
	...
	public function spending_form($oppID, $bID)
    {
		...
		$data['usertypes']	= $this->usertype_model->getUsertypeInfo($oppID,$bID);
		
		$data['oppID'] 	= $oppID;
		$data['bID']	= $bID;
		$this->load->View("usertype/spending_form", $data);
	
	
    }
	
	public function doSetupUsertype()
    {
		...
		if(true)
        {
			...
			$subdivid  = $this->input->post('subdivisionId');
			
			$array = array('NAME'=> $name, 'IS_ADMIN'=> $isAdmin, 'DIVISION_ID'=> $divid, 'SUBDIVISION_ID'=> $subdivid);
			
			$result = $this->usertype_model->setupUsertype($array);
			
			if($result > 0)
			{
				$this->session->set_flashdata('success', 'Usertype Set Submitted Successfully');
			}
			else
			{
				$this->session->set_flashdata('error', 'Usertype creation failed');
			}
	...
	public function doEditUsertype($id)
    {
		...
		if(true)
        {
			...
			$result = $this->usertype_model->updateData($array, $id);
	...
	public function doAddSpending($source)
    {
		...
		{
			$config['upload_path']          = 'docs/usertype';
			...
			//check remaining usertype
			$balance = $this->usertype_model->checkBalance($cost, $oppbID);
			...
			$result = $this->usertype_model->addNewSpending($spending, $file, $balance, $date);
