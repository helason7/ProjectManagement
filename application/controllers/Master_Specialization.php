 ...
 */
class Master_Specialization extends BaseController
{
    /**
    ...
    public function index()
    {
		...
		if($this->isAdmin() == true)
        {
			...
			redirect('Specialization/specializationList/');
	...
	public function specializationList()
    {
		...
		$data['specializations']	= $this->specialization_model->getListSpecialization();
			
		
		$this->loadViews("specialization/list_specialization", $this->global, $data);
		
    }
	
	public function setup_specialization()
    {
        $this->global['pageTitle'] = 'Setup Master Specialization';
		...
		if(true)
        {
			...
			$this->loadViews("specialization/setup", $this->global, $data);
	...
	public function delete_data($id){
	    ...
	    $id = array('ID' => $id);
	    $this->load->model('specialization_model');
	    $this->specialization_model->Delete('M_SPESIALIZATION', $id);
	    redirect('Master_Specialization/specializationList/');
	}
	...
	public function update_data($id){
		 $this->global['pageTitle'] = 'Dashboard';
  		...
  		if($this->isAdmin() == true){
  			$data['specializations'] = $this->specialization_model->getDataById($id);
  		$this->loadViews("specialization/edit_form", $this->global, $data);
  		} else
          {
               redirect('/Main');
          }
	...
	public function detail_specialization($name)
    {
        $this->global['pageTitle'] = 'Setup Master Specialization';
		...
		if(true)
        {
			...
			$data['specialization']	= $this->specialization_model->getDetailSpecialization($name);
			
			
			$this->load->View("specialization/specialization_detail", $data);
	...
	public function set_detail_specialization($name)
    {
        $this->global['pageTitle'] = 'Setup Master Specialization';
		...
		$this->load->View("specialization/set_specialization_detail", $data);
	
	
	
    }
	
	public function usage_detail_specialization($oppID, $bName)
    {
		...
		$data['specializations']	= $this->specialization_model->getSpecializationUsage($oppID,$bName);
		
		
		$this->load->View("specialization/usage_specialization_detail", $data);
	...
	public function spending_form($oppID, $bID)
    {
		...
		$data['specializations']	= $this->specialization_model->getSpecializationInfo($oppID,$bID);
		
		$data['oppID'] 	= $oppID;
		$data['bID']	= $bID;
		$this->load->View("specialization/spending_form", $data);
	
	
    }
	
	public function doSetupSpecialization()
    {
		...
		if(true)
        {
			...
			$result = $this->specialization_model->setupData($array);
			if($result > 0)
			{
				$this->session->set_flashdata('success', 'Specialization Set Submitted Successfully');
			}
			else
			{
				$this->session->set_flashdata('error', 'Specialization creation failed');
			}
	...
	public function doEditSpecialization($Id)
	{
		if($this->isAdmin() == TRUE)
        {
			//$this->loadViews("project/list_projects", $this->global, $data);
			...
			$result = $this->specialization_model->updateData($array, $Id);
	...
	public function doAddSpending($source)
    {
		...
		{
			$config['upload_path']          = 'docs/specialization';
			...
			//check remaining specialization
			$balance = $this->specialization_model->checkBalance($cost, $oppbID);
			...
			$result = $this->specialization_model->addNewSpending($spending, $file, $balance, $date);
