...
class Specialization_model extends CI_Model
{
	...
	function setupSpecialization($p_order, $p_name, $c_order, $c_name, $set_name)
    {
        $this->db->trans_start();
		$insert_id = '';
		...
		{
			$specialization = array('NAME'=> $p_name[$key], 'PARENT'=> 0, 'ORDER_NO'=> $val, 'STATUS'=> 'ACTIVE','SET_NAME'=> $set_name);
			$this->db->insert('M_SPESIALIZATION', $specialization);
		}
		...
		if(count($c_order) > 0)
		{
			foreach($c_order as $key=>$val)
			{
				...
				{
					$specialization = array('NAME'=> $c_name[$key][$key_], 'PARENT'=> $parent_id, 'ORDER_NO'=> $val_, 'STATUS'=> 'ACTIVE','SET_NAME'=> $set_name);
					$this->db->insert('M_SPESIALIZATION', $specialization);
				}
			}
		}
		...
		$query = $this->db->get('M_SPESIALIZATION'); 
		$res = $query->row();			
    ...
    {
        $this->db->trans_start();
        $this->db->insert('M_SPESIALIZATION', $array);
    ...
    function updateData($array, $Id)
    {
      ...
      if($this->db->update('M_SPESIALIZATION', $array)){
        $this->db->trans_complete();
        return 1;
      }else{
        return 0;
	...
	function getListSpecialization()
	{
        ...
        $this->db->from("M_SPESIALIZATION A");
	...
	function getListActiveSpecialization()
	{
	...
	function getListCompSpecialization($idOpp)
	{
		$this->db->select("A.*");
        $this->db->from("M_SPESIALIZATION A");
	...
	function getSpecializationInfo($oppID, $bID)
	{
	...
	function addNewSpending($spending, $file, $balance, $date)
    {
		...
		{
			$name = $file['file_name'];
			$link = base_url().'docs/specialization/'.$file['file_name'];
			$date = date('Y-m-d');
	...
	function getDetailSpecialization($name)
	{
		$this->db->select("*");
        $this->db->from("M_SPESIALIZATION");
		...
		foreach($result as $key=>$val)
		{
			if($val->PARENT == 0)
			{
				$specialization[$val->ID]['TYPE_NAME'] = $val->NAME;
				$specialization[$val->ID]['ORDER'] = $val->ORDER_NO;
				$specialization[$val->ID]['COMP_NAME'][] = "";
			}
			else
			{
				$specialization[$val->PARENT]['COMP_NAME'][] = $val->NAME."|".$val->ORDER_NO."|".$val->ID;
			}
		}
		 return $specialization;
	}
	
	function getSpecializationUsage($oppID,$bName)
	{
		$this->db->select("B.*, NVL(O.AMOUNT, 0) AS AMOUNT, O.USAGE, O.REMAINDER");
        $this->db->from("M_SPESIALIZATION B");
		...
		foreach($result as $key=>$val)
		{
			if($val->PARENT == 0)
			{
				$specialization[$val->ID]['TYPE_NAME'] = $val->NAME;
				$specialization[$val->ID]['ORDER'] = $val->ORDER_NO;
				$specialization[$val->ID]['AMOUNT'] = $val->AMOUNT;
				$specialization[$val->ID]['USAGE'] = $val->USAGE;
				$specialization[$val->ID]['REMAINDER'] = $val->REMAINDER;
				$specialization[$val->ID]['COMP_NAME'][] = "";
			}
			else
			{
				$specialization[$val->PARENT]['COMP_NAME'][] = $val->NAME."|".$val->ORDER_NO."|".$val->ID."|".$val->AMOUNT."|".$val->USAGE."|".$val->REMAINDER;
			}
		}
		 return $specialization;
	}
	...
	function getSpendingOpp($oppID)
	{
		...
		$this->db->join("M_SPESIALIZATION B","O.BUDGET_ID = B.ID");
	...
	function getListAllocatedSpecialization()
	{
	...
	function getListAllocatedSpecializationPost()
	{
